class Solution(object):
    def maxActiveSectionsAfterTrade(self, s, queries):
        class SparseTable(object):
            def __init__(self, arr, fn):
                self.fn = fn
                n = len(arr)
                self.bit_length = [0]
                k = n.bit_length() - 1 if n > 0 else -1
                for i in range(k + 1):
                    self.bit_length.extend(i + 1 for _ in range(min(1 << i, (n + 1) - len(self.bit_length))))
                self.st = [[0] * n for _ in range(k + 1)]
                if not self.st:
                    return
                self.st[0] = arr[:]
                for i in range(1, k + 1):
                    for j in range((n - (1 << i)) + 1):
                        self.st[i][j] = fn(self.st[i - 1][j], self.st[i - 1][j + (1 << (i - 1))])

            def query(self, l, r):
                i = self.bit_length[r - l + 1] - 1
                return self.fn(self.st[i][l], self.st[i][r - (1 << i) + 1])

        lookup = [-1] * len(s)
        idxs = []
        cnt1 = 0
        for i, ch in enumerate(s):
            if ch == '0':
                if i - 1 >= 0 and s[i - 1] == '0':
                    idxs[-1][1] += 1
                else:
                    idxs.append([i, 1])
            else:
                cnt1 += 1
            lookup[i] = len(idxs) - 1

        if not idxs:
            return [cnt1] * len(queries)

        arr = [0] * (len(idxs) - 1)
        for i in range(len(idxs) - 1):
            arr[i] = idxs[i][1] + idxs[i + 1][1]
        st = SparseTable(arr, max)

        result = [cnt1] * len(queries)
        for qi, (l, r) in enumerate(queries):
            left = lookup[l] + 1
            right = lookup[r] - (1 if s[r] == '0' else 0)
            left_cnt = idxs[lookup[l]][1] - (l - idxs[lookup[l]][0]) if lookup[l] != -1 else -1
            right_cnt = r - idxs[lookup[r]][0] + 1 if lookup[r] != -1 else -1

            if left <= right - 1:
                result[qi] = max(result[qi], cnt1 + st.query(left, right - 1))
            if s[l] == '0' and s[r] == '0' and lookup[l] + 1 == lookup[r]:
                result[qi] = max(result[qi], cnt1 + left_cnt + right_cnt)
            if s[l] == '0' and lookup[l] + 1 <= right:
                result[qi] = max(result[qi], cnt1 + left_cnt + idxs[lookup[l] + 1][1])
            if s[r] == '0' and left <= lookup[r] - 1:
                result[qi] = max(result[qi], cnt1 + right_cnt + idxs[lookup[r] - 1][1])

        return result